...
func execNopCloser(_ int, p *igo.Context) {
	args := p.GetArgs(1)
	ret0 := ioutil.NopCloser(toType0(args[0]))
	p.Ret(1, ret0)
}

func execReadAll(_ int, p *igo.Context) {
	args := p.GetArgs(1)
	ret0, ret1 := ioutil.ReadAll(toType0(args[0]))
	p.Ret(1, ret0, ret1)
}

func execReadDir(_ int, p *igo.Context) {
	args := p.GetArgs(1)
	ret0, ret1 := ioutil.ReadDir(args[0].(string))
	p.Ret(1, ret0, ret1)
}

func execReadFile(_ int, p *igo.Context) {
	args := p.GetArgs(1)
...
func execTempFile(_ int, p *igo.Context) {
	args := p.GetArgs(2)
	ret0, ret1 := ioutil.TempFile(args[0].(string), args[1].(string))
	p.Ret(2, ret0, ret1)
}

func execWriteFile(_ int, p *igo.Context) {
	args := p.GetArgs(3)
...
var I = igo.NewGoPackage("io/ioutil")
