...
func execmCondWait(_ int, p *igo.Context) {
	args := p.GetArgs(1)
	args[0].(*sync.Cond).Wait()
	p.PopN(1)
}

func execmCondSignal(_ int, p *igo.Context) {
	args := p.GetArgs(1)
	args[0].(*sync.Cond).Signal()
	p.PopN(1)
}

func execmCondBroadcast(_ int, p *igo.Context) {
	args := p.GetArgs(1)
	args[0].(*sync.Cond).Broadcast()
	p.PopN(1)
}

func execmMapLoad(_ int, p *igo.Context) {
	args := p.GetArgs(2)
	ret0, ret1 := args[0].(*sync.Map).Load(args[1])
	p.Ret(2, ret0, ret1)
}

func execmMapStore(_ int, p *igo.Context) {
	args := p.GetArgs(3)
	args[0].(*sync.Map).Store(args[1], args[2])
	p.PopN(3)
}

func execmMapLoadOrStore(_ int, p *igo.Context) {
	args := p.GetArgs(3)
	ret0, ret1 := args[0].(*sync.Map).LoadOrStore(args[1], args[2])
	p.Ret(3, ret0, ret1)
}

func execmMapDelete(_ int, p *igo.Context) {
	args := p.GetArgs(2)
	args[0].(*sync.Map).Delete(args[1])
	p.PopN(2)
}

func execmMapRange(_ int, p *igo.Context) {
	args := p.GetArgs(2)
	args[0].(*sync.Map).Range(args[1].(func(key interface{}, value interface{}) bool))
	p.PopN(2)
}

func execmMutexLock(_ int, p *igo.Context) {
	args := p.GetArgs(1)
	args[0].(*sync.Mutex).Lock()
	p.PopN(1)
}

func execmMutexUnlock(_ int, p *igo.Context) {
	args := p.GetArgs(1)
...
func execNewCond(_ int, p *igo.Context) {
	args := p.GetArgs(1)
	ret0 := sync.NewCond(toType0(args[0]))
	p.Ret(1, ret0)
}

func execmOnceDo(_ int, p *igo.Context) {
	args := p.GetArgs(2)
	args[0].(*sync.Once).Do(args[1].(func()))
	p.PopN(2)
}

func execmPoolPut(_ int, p *igo.Context) {
	args := p.GetArgs(2)
	args[0].(*sync.Pool).Put(args[1])
	p.PopN(2)
}

func execmPoolGet(_ int, p *igo.Context) {
	args := p.GetArgs(1)
	ret0 := args[0].(*sync.Pool).Get()
	p.Ret(1, ret0)
}

func execmRWMutexRLock(_ int, p *igo.Context) {
	args := p.GetArgs(1)
	args[0].(*sync.RWMutex).RLock()
	p.PopN(1)
}

func execmRWMutexRUnlock(_ int, p *igo.Context) {
	args := p.GetArgs(1)
	args[0].(*sync.RWMutex).RUnlock()
	p.PopN(1)
}

func execmRWMutexLock(_ int, p *igo.Context) {
	args := p.GetArgs(1)
	args[0].(*sync.RWMutex).Lock()
	p.PopN(1)
}

func execmRWMutexUnlock(_ int, p *igo.Context) {
	args := p.GetArgs(1)
	args[0].(*sync.RWMutex).Unlock()
	p.PopN(1)
}

func execmRWMutexRLocker(_ int, p *igo.Context) {
	args := p.GetArgs(1)
	ret0 := args[0].(*sync.RWMutex).RLocker()
	p.Ret(1, ret0)
}

func execmWaitGroupAdd(_ int, p *igo.Context) {
	args := p.GetArgs(2)
	args[0].(*sync.WaitGroup).Add(args[1].(int))
	p.PopN(2)
}

func execmWaitGroupDone(_ int, p *igo.Context) {
	args := p.GetArgs(1)
	args[0].(*sync.WaitGroup).Done()
	p.PopN(1)
}

func execmWaitGroupWait(_ int, p *igo.Context) {
	args := p.GetArgs(1)
